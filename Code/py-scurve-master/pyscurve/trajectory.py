...
#from articulated_arm import serial_manipulator
...
#    fig = plt.figure(0)
    ...
    for i, profile in zip(range(dof), r_profiles):
        ...
        plt.xlim()
        plt.ylim()
        #print("speed profile",profile[SPEED_ID][:])
        #print("max speed",max(profile[SPEED_ID][:]))
        #print("total time",len(profile[SPEED_ID][:]))
